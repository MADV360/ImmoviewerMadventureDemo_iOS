...
@interface KxAVPacket()
@property (readwrite, nonatomic) AVPacket *packet;
@end

@implementation KxAVPacket
@end
...
@interface KxMovieDecoder () {
    ...
    BOOL                _isUsedForEncoder;
    NSMutableArray      *_readPackets;
...
#ifdef DUMPYUV
    FILE                *dumpYUVFile;
    int                 dumpYUVCount, dumpedYUVSize;
#endif
...
- (CGFloat) positionAudio
{
    return _positionAudio;
}
...
- (void) setPosition: (CGFloat)seconds
{
    ...
    if (_videoStream != -1) {
        ...
        _bufferedPacketCount = 0;
        if ( _readPackets != nil) {
            [_readPackets removeAllObjects];
        }        
...
///!!!qiudong 20160414
    ...
    @synchronized(self){
        ...
        const char * a =[filename UTF8String];
        if (!dumpH264File) {
            dumpH264File = fopen(a, "wb");
            dumpCount = 1;
            dumpedSize = 0;
        }
#endif
#ifdef DUMPYUV
        NSString* yuvfileOutputPath = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"];
        NSString* baseYUVFilename = @"dumpYUV";
        NSString* yuvfilename = [yuvfileOutputPath stringByAppendingPathComponent:baseYUVFilename];
        const char * yuv =[yuvfilename UTF8String];
        if (!dumpYUVFile) {
            dumpYUVFile = fopen(yuv, "wb");
            dumpYUVCount = 1;
            dumpedYUVSize = 0;
        }
#endif
...
-(void) closeFile
{
    ...
    @synchronized(self) {
        ...
        if (_formatCtx) {
            ...
            if(_formatCtx->gpsx_data) {
                free(_formatCtx->gpsx_data);
                _formatCtx->gpsx_data = NULL;
            }
            
            if(_formatCtx->gyro_data) {
                free(_formatCtx->gyro_data);
                _formatCtx->gyro_data = NULL;
            }
            
            if(_formatCtx->caif_data) {
                free(_formatCtx->caif_data);
                _formatCtx->caif_data = NULL;
            }
            
            if(_formatCtx->cut_data) {
                free(_formatCtx->cut_data);
                _formatCtx->cut_data = NULL;
            }
...
- (KxAudioFrame *) handleAudioFrame
{
    ...
    //NSLog(@"a orig samples:%d, proc frames:%lu, length:%lu", _audioFrame->nb_samples, (unsigned long)numElements, (unsigned long)data.length);
    ...
    if (frame.duration == 0) {
        ...
        if (!_isUsedForEncoder)
            ...
            frame.duration = frame.samples.length / (sizeof(float) * numChannels * _audioFrame->sample_rate);
    }
...
- (int) getCaifSize
{
    if (_formatCtx != NULL)
    {
        NSLog(@"Caif: getCaifSize = %d", _formatCtx->caif_size);
        return _formatCtx->caif_size;
    }
    else
        return 0;
}
...
#define FPS120_BOTTOM_1920x480  9
    ...
    uint8_t* caifData = self.getCaifData;
    MADV_MP4_CAMERA_INFO_t* madvMp4CameraInfo = (MADV_MP4_CAMERA_INFO_t*) caifData;
    if (madvMp4CameraInfo)
    {
        const char* resolutionString = madvMp4CameraInfo->Res;
        int resolutionID = resolutionString[0] - '0';
        if (resolutionID >= FPS30_3456x1728 && resolutionID <= FPS120_BOTTOM_1920x480)
        {
            return resolutionID;
        }
    }
    return FPS30_3456x1728;
...
{
    if (_fps > 0)
        return (CGFloat)_bufferedPacketCount / (CGFloat)_fps;
    else
        return 0;
}
...
- (NSArray *) decodeFrames: (CGFloat) minDuration
{
    ...
    while (!finished) {
        NSLog(@"keep looping");
        AVPacket* packet = [self readFramesFromBuffer];
        if ( packet == nil ){//|| packet->size <= 0) {
            NSLog(@"#Codec# KxMovieDecoder decodeFrames: red null avpacket @ %@", self);
            //_isEOF = YES;
            break;
        }
        NSLog(@"consume packet of stream %d, size: %d, total: %ld", packet->stream_index, packet->size, _readPackets.count);
        
        if (waitNextKeyFrame) {
            if (packet->flags & AV_PKT_FLAG_KEY) {
                waitNextKeyFrame = false;
                av_log(NULL, AV_LOG_ERROR, "found next Key frame,stop skipping frames");
                NSLog(@"found next Key frame,stop skipping frames");
            } else {
                av_log(NULL, AV_LOG_ERROR, "skipping frames");
                NSLog(@"skipping frames");
                continue;
            }
        }
        ...
        if (packet->stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                int gotframe = 0;
                @synchronized(self) {
                    NSLog(@"going to deocde... %d x %d", _videoCodecCtx->width, _videoCodecCtx->height);
                    if(_videoCodecCtx != nil) {
#ifdef DUMPH264
                        if (dumpH264File) {
                            fwrite(packet->data, 1, packet->size, dumpH264File);
                            NSLog(@"dumped %dth frame, started from %d to %d", dumpCount, dumpedSize, dumpedSize + packet->size - 1 );
                            dumpCount++;
                            dumpedSize += packet->size;
                        }
#endif
                        len = avcodec_decode_video2(_videoCodecCtx,
                                                    ...
                                                    &gotframe,
                                                    packet);
                        NSLog(@"pkt %d frame %d", packet->size, len);
                        //                ///!!!For Debug
                        //                NSTimeInterval afterDecode = [NSDate timeIntervalSinceReferenceDate];
                        //                totalDecodeTime += (afterDecode - beforeDecode);
                        ...
                        NSLog(@"not decoding because of _videoCodecCtx = nil");
                    }
...
#ifdef DUMPYUV
                        if (dumpYUVFile) {
                            fwrite(_videoFrame, 1, packet->size, dumpYUVFile);
                            NSLog(@"dumped %dth frame, started from %d to %d", dumpYUVCount, dumpedYUVSize, dumpedYUVSize + packet->size - 1 );
                            dumpYUVCount++;
                            dumpedYUVSize += packet->size;
                        }
#endif
...
- (boolean_t) readPackets: (CGFloat) minDuration
{
    ...
    __block CGFloat readDuration = 0;
...
//            fwrite(packet.data, 1, packet.size, dumpH264File);
        ...
        NSLog(@"%@ readPackets add one packet %lld", self, packet.pts);
        KxAVPacket *avpacket = [[KxAVPacket alloc] init];
        avpacket.packet = newpacket;
        if (_readPackets == nil)
            _readPackets = [NSMutableArray array];
        
        @synchronized(_readPackets) {
            [_readPackets addObject: avpacket];
        }
        if (newpacket->stream_index == _videoStream ||
            (newpacket->stream_index == _audioStream && _videoStream == -1)) {
            _bufferedPacketCount++;
            readPacketCount++;
        }
        if (readPacketCount/_fps >= _minDuration)
            finished = YES;
        //NSLog(@"read packet of stream %d, size: %d, total:%ld", avpacket.packet->stream_index, avpacket.packet->size, _readPackets.count);
    }
...
- (AVPacket*) readFramesFromBuffer
{
    KxAVPacket *avpacket;
    @synchronized (_readPackets) {
        if (_readPackets.count > 0) {
            avpacket = (KxAVPacket*)_readPackets[0];
            [_readPackets removeObjectAtIndex:0];
            if (avpacket.packet->stream_index == _videoStream ||
                (avpacket.packet->stream_index == _audioStream && _videoStream == -1)) {
                _bufferedPacketCount--;
            }
            /*
            if(packet.stream_index == _videoStream ||
                (packet.stream_index == _audioStream && _videoStream == -1)) {
                _readVideoDuration -= packet.duration;
            }*/
        }
    }
    return avpacket.packet;
}
...
- (void) setBufferDuration: (CGFloat) minDuration maxDuration: (CGFloat)maxDuration
{
    ...
    if (_videoStream != -1) {
        if (self.fps > 0) {
           _minBufferedFrameCount = _minDuration / (1.0 / self.fps);
           _maxBufferedFrameCount = _maxDuration / (1.0 / self.fps);
        } else {
            _minBufferedFrameCount = 0;
            _maxBufferedFrameCount = 150;
        }
    } else if (_audioStream != -1) {
        CGFloat audioFrameRate = 48.0;
        _minBufferedFrameCount = _minDuration / (1.0 / audioFrameRate);
        _maxBufferedFrameCount = _maxDuration / (1.0 / audioFrameRate);
    }
}
