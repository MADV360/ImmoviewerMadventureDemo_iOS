...
static int mov_read_caif(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    c->fc->caif_size = atom.size;
    
    if (c->fc->caif_size > 0) {
        if (!c->fc->caif_data)
            c->fc->caif_data = malloc(c->fc->caif_size);
        avio_read(pb, c->fc->caif_data, c->fc->caif_size);
    }
    av_log(c->fc, AV_LOG_ERROR, "caif size: %"PRId64"\n", c->fc->caif_size);
    return 0;
}
